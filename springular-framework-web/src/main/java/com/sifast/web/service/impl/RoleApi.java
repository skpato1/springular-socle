...
public class RoleApi implements IRoleApi {
    ...
    private int getDefaultAuthorityByDesignation(String designation) {
        return authorityService.findByDesignation(designation, LogicalDeleteEnum.FALSE).getId();
    }
    ...
    public ResponseEntity<Object> getAllRoles() {
        httpStatus = HttpStatus.OK;
        List<Role> roles = roleService.findByIsDeleted();
        if (roles.isEmpty()) {
    ...
    public ResponseEntity<Object> updateRole(
        ...
        if (!bindingResult.hasFieldErrors()) {
            ...
            if (role.isPresent()) {
                ...
                List<Authority> authorities = authorityService.findByIdIn(roleDto.getAuthoritiesId(), LogicalDeleteEnum.FALSE);
